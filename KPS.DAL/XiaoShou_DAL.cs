...
namespace KPS.DAL
{
	...
	/// 数据访问类:PSI_XiaoShou
	/// </summary>
	public partial class XiaoShou_DAL
	{
		public XiaoShou_DAL()
		{}
		...
		public int GetMaxId()
		{
		return DbHelperOleDb.GetMaxID("ID", "PSI_XiaoShou"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from PSI_XiaoShou");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from PSI_XiaoShou ");
			strSql.Append(" where ID=@ID");
			OleDbParameter[] parameters = {
					new OleDbParameter("@ID", OleDbType.Integer,4)
			};
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select ID,p_date,p_cpmc,p_ggxh,p_clmc,p_ph,p_dw,p_sl1,p_mjph,p_zzs,p_zczh,p_gys,p_sl2,p_jsr,DataType,RemarkInfo,GJID ");
			strSql.Append(" FROM PSI_XiaoShou ");
			if(strWhere.Trim()!="")
			{
        ...
        /// <returns></returns>
        public DataSet GetTabDataBySQL(string _SQLwhere)
        {
            return DbHelperOleDb.Query(_SQLwhere);
        }
