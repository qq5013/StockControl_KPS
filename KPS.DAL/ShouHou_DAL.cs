...
namespace KPS.DAL
{
	...
	/// 数据访问类:PSI_ShouHou
	/// </summary>
	public partial class ShouHou_DAL
	{
		public ShouHou_DAL()
		{}
		...
		public int GetMaxId()
		{
		return DbHelperOleDb.GetMaxID("ID", "PSI_ShouHou"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from PSI_ShouHou");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(KPS.Model.ShouHouInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update PSI_ShouHou set ");
            ...
            strSql.Append("y_managertype=@y_managertype,");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from PSI_ShouHou ");
			strSql.Append(" where ID=@ID");
			OleDbParameter[] parameters = {
					new OleDbParameter("@ID", OleDbType.Integer,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public KPS.Model.ShouHouInfo DataRowToModel(DataRow row)
		{
            KPS.Model.ShouHouInfo model = new KPS.Model.ShouHouInfo();
			if (row != null)
			{
                ...
                if (row["y_managertype"] != null && row["y_managertype"].ToString() != "")
                {
                    if ((row["y_managertype"].ToString() == "1") || (row["y_managertype"].ToString().ToLower() == "true"))
                    {
                        model.y_managertype = true;
                    }
                    else
                    {
                        model.y_managertype = false;
                    }
                }
                if (row["DataType"] != null)
				{
                    model.DataType =Convert.ToInt32(row["y_fhrqz"]);
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select ID,y_date,y_pm,y_ggxh,y_dw,y_sl,y_sccj,y_cpdm,y_ghdw,y_cpzczh,y_scph,y_mjph,y_yxq,y_isHGZ,y_zlqk,y_zgy,y_fhrqz,y_managertype,DataType ");
			strSql.Append(" FROM PSI_ShouHou ");
			if(strWhere.Trim()!="")
			{
