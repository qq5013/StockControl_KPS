...
namespace CSharpWin
{
    ...
    public class SkinForm : Form
    {
        ...
        {
            get {
                if (_renderer is IPSSkinRender)
                {
                    return ((IPSSkinRender)_renderer).TitleImage.Height;
                    //float rate = (float)((IPSSkinRender)_renderer).TitleImage.Width / (float)this.CaptionRect.Width;
                    //return (int)(((IPSSkinRender)_renderer).TitleImage.Height * rate);
                }
                else
                {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (Renderer is IPSSkinRender)
            {
                foreach (ImageButton button in ((IPSSkinRender)Renderer).Buttons)
                {
                    button.MouseMove(PointToClient(MousePosition).X, PointToClient(MousePosition).Y);
                }
            }
        }
        ...
        protected override void OnClick(EventArgs e)
        {
            ...
            if (Renderer is IPSSkinRender)
            {
                foreach (ImageButton imgButton in ((IPSSkinRender)Renderer).Buttons)
                {
                    Point p = PointToClient(MousePosition);
                    if (imgButton.GetButtonRectangle().Contains(p.X, p.Y))
                    {
                        imgButton.MouseClick();
                        break;
                    }
                }
            }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (renderer is IPSSkinRender)
            {
                ((IPSSkinRender)renderer).DrawSkinFormButtons();
                //DrawButtons(e);
            }
        ...
        private void WmNcHitTest(ref Message m)
        {
            ...
            if (point.Y < CaptionHeight)
            {
                if (!ControlBoxManager.CloseBoxRect.Contains(point) &&
                    ...
                    !ControlBoxManager.MinimizeBoxRect.Contains(point)&& !ButtonContain(point))
                {
                    m.Result = new IntPtr(
        ...
        private bool ButtonContain(Point point)
        {
            if (Renderer is IPSSkinRender)
            {
                foreach (ImageButton button in ((IPSSkinRender)Renderer).Buttons)
                {
                    if (button.GetButtonRectangle().Contains(point))
                    {
                        return true;
                    }
                }
